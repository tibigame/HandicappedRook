...
def large_test(move_list):
    ...
    for i, move in enumerate(move_list):
        m_d = kifu.move(move)
        senkei.move(m_d, i+1)
    return senkei.get_large_classification()
...
class WhiteRangingRookTest(unittest.TestCase):
    def test_1(self):
        ...
        result = large_test(make_kifu_list(kifu_sfen))
        self.assertEqual("後手角交換向かい飛車", result)
...
class DoubleRangingRookTest(unittest.TestCase):
    def test_1(self):
        ...
        result = large_test(make_kifu_list(kifu_sfen))
        self.assertEqual("先手三間飛車後手向かい飛車", result)
